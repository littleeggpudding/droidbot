...
class Uiautomator2_Helper:
    ...
    def get_instance(self, view_dict):
        """
        为view分配instance ID
        基于className和resourceId的组合进行计数
        """
        class_name = view_dict.get('class')
        resource_id = view_dict.get('resource_id')
        text = view_dict.get('text')
        content_description = view_dict.get('content_description')
        
        flag = False
        i = 0
        while i < len(self.classname_list):
            if (self.classname_list[i] == class_name and 
                self.resourceid_list[i] == resource_id and
                self.text_list[i] == text and
                self.content_description_list[i] == content_description):
                flag = True
                self.num_list[i] = self.num_list[i] + 1
                instance_id = self.num_list[i]
                break
            i += 1
            
        if flag is False:
            self.classname_list.append(class_name)
            self.resourceid_list.append(resource_id)
            self.text_list.append(text)
            self.content_description_list.append(content_description)
    ...
    def xml_to_dict(self, element) -> Dict:
        ...
        result["instance"] = self.get_instance(result)
